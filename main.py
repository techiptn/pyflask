...
def load_user(user_id):
    return UserPost.query.get(int(user_id))
...
class Comment(db.Model):
    ...
    commentor = relationship("UserPost", back_populates="comments")
...
def admin_only(f):
    ...
    def decorated_function(*args, **kwargs):
        if current_user.id != 4 or not current_user.is_authenticated:
            return abort(403)
...
def register():
    ...
    if form.validate_on_submit():
        emailexist = UserPost.query.filter_by(email=form.email.data).first()
        if emailexist:
            flash("You signed up with this email, please log in instead.")
...
def login():
    ...
    if form.validate_on_submit():
        ...
        user = UserPost.query.filter_by(email=email).first()
        # Email doesn't exist
        if not user:
